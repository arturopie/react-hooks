...
`useEffect` can be used for lifecycle events, when you want a function to execute when state, props or context changes in a component. Use it to syncronized the state of the world with the state of our React app (e.g. localstorage, API requests, etc)

Extracting custom hooks is very easy and helps with reusability. How about testeability? Does it help with it too? Is it easy/recommended to mock React hooks in tests?

Now, I better understand the applicability of useRef, as I thought it was only meant to be used for DOM or React components. The fact that you can use it to hold any value without having to re render is very cool.

When you want to hold state, you can use `useState` or `useRef`. When to use useState vs useRef:
* `useState` if you want the component to rerender when state change
* `useRef` if you don't want the component to rerender when state change
Is there any other case when I should use one over the other?
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=02%3A%20useEffect%3A%20persistent%20state&em=arturop%40nulogy.com).
