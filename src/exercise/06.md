...
I learned about the subtle semantic difference when handling promise errors.

Good practise: use status instead of booleans to control status of async request. This will help prevent bugs, for example, I often see ppl forgetting to handle one of the 4 status, by enumerating them, it should reduce this risk.
Good idea to standardize on the following statuses:
* idle
* pending
* resolved
* rejected
...
* I don't like the idea of using pokemonName as key to reset the ErrorBoundary as that will unmount/mount all its children as well, so everytime pokemonName changes, it will run all lazy initializers and useEffects with `[]` dependencies. Is this really the best way? (see exercise/06.extra-5.js for an example)
* How do you reset a top level ErrorBoundary?

The Error Boundary in ECO could be better.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=06%3A%20useEffect%3A%20HTTP%20requests&em=arturop%40nulogy.com).
