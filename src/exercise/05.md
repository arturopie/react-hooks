...
Rule:
* Do not query DOM elements directly, use `ref` property and useRef to get the DOM element created by React

* Use `useEfect` to operate on the DOM element. If the operation has to happen on mount and/or unmount, make sure the `useEfect` dependency is an empty array
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=05%3A%20useRef%20and%20useEffect%3A%20DOM%20interaction&em=arturop%40nulogy.com).
