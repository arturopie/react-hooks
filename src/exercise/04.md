...
Order for implementing UI:
1. Figure out UI you need (as just stories)
2. Figure out state you need to manage
3. Add interoperability

Rules:
* Do not mutate the state directly, but make a copy and pass it to the setState function

NOTE: PropTypes are not used in the examples anymore

Questions:
1. Why define renderSquare as a function rather than another component?

When completing credit 3, I ran into a bug that made me waste some debugging time. However, with Typescript I would have detected the bug quicker.

I'm a big of splitting big refactorings into small steps while keeping the test greens after each step (inspired by the book Refactoring by Martin Fowler). I spent a fair amount of time figuring out how to do the refactoring from classes to functions and hooks on small steps. It was hard to figure it out, but worth it. I had to use a lot of `bind` calls in the intermediate steps.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=04%3A%20useState%3A%20tic%20tac%20toe&em=arturop%40nulogy.com).
